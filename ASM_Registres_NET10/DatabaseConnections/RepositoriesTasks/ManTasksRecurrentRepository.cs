...
{
    public class ManTasksRecurrentRepository
    {
            ...
            using (var conn = new NpgsqlConnection(DatabaseCredentials.ConnectionStringLocal))
            {
                ...
                using (var cmd = new NpgsqlCommand(
                    @"SELECT id, day, type, title, description, comments, assigned_person, priority, completed, shift 
                      FROM tasks_schema.maintenance_tasks 
        ...
        public Tasks GetTareaById(int id)
        {
            ...
            using (var conn = new NpgsqlConnection(DatabaseCredentials.ConnectionStringLocal))
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand(
                    "SELECT id, day, type, title, description, comments, assigned_person, priority, completed, shift FROM tasks_schema.maintenance_tasks WHERE id = @id", conn))
                {
        ...
        public void AddTask(Tasks newTask)
        {
            using (var conn = new NpgsqlConnection(DatabaseCredentials.ConnectionStringLocal))
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand(
                    @"INSERT INTO tasks_schema.maintenance_tasks 
        ...
        public void DeleteTask(int id)
        {
            using (var conn = new NpgsqlConnection(DatabaseCredentials.ConnectionStringLocal))
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand(
                    "DELETE FROM tasks_schema.maintenance_tasks WHERE id = @id", conn))
                {
                    cmd.Parameters.AddWithValue("id", id);
                    cmd.ExecuteNonQuery();
                }
        ...
        public void UpdateTask(Tasks updatedTask)
        {
            using (var conn = new NpgsqlConnection(DatabaseCredentials.ConnectionStringLocal))
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand(
                    @"UPDATE tasks_schema.maintenance_tasks 
                     SET day = @dia, 
                      type = @tipo,     
                      title = @titulo, 
        ...
        public List<Tasks> GetTareasByFecha(DateTime fecha)
        {
            ...
            using (var conn = new NpgsqlConnection(DatabaseCredentials.ConnectionStringLocal))
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand(
                    "SELECT id, day, type, title, description, comments, assigned_person, priority, completed, shift FROM tasks_schema.maintenance_tasks WHERE day = @fecha", conn))
                {
        ...
        public void UpdateCompletadaEstado(int taskId, bool completada)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("UPDATE tasks_schema.maintenance_tasks SET completed = @completada WHERE id = @id", conn))
                {
            ...
            using (var conn = new NpgsqlConnection(DatabaseCredentials.ConnectionStringLocal))
            {
                ...
                using (var cmd = new NpgsqlCommand(
                    @"SELECT nombre_usuario 
                      ...
                      WHERE departamento = 'Mantenimiento' AND activo = 'Activo'", conn))
                {
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
